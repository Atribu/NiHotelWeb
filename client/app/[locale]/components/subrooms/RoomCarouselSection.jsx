...
export default function RoomCarouselSection({
  ...
  return (
    <section className="py-4 lg:py-10 bg-white z-50">
      <div className="max-w-[1440px] mx-auto px-4 grid-cols-1 lg:grid lg:grid-cols-3 gap-6 items-center shadow-lg w-[96%] pb-5 lg:w-[100%]">
        ...
        {descriptions.map((p, i) => (
          <p
            ...
            className="
       flex lg:hidden
       text-gray-700 text-[12px] lg:text-sm mb-6
       overflow-hidden
       line-clamp-4 "
          >
            {p}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:col-span-2 lg:hidden ">
          {slideIndices.map((i, imageIdx) => (
            <div
              key={i}
              className={`relative h-72 lg:h-80 xl:h-96 overflow-hidden shadow-lg cursor-pointer ${
                imageIdx === 1 ? "hidden md:block" : ""
              }`}
        ...
        <div className="space-y-2 md:space-y-4 lg:space-y-6 text-center items-center justify-center flex flex-col lg:items-start lg:justify-start lg:text-start">
          <h3 className="hidden lg:flex text-[24px] lg:text-3xl font-['Cormorant_Garamond'] font-semibold">
            {title}
          </h3>
          {descriptions.map((p, i) => (
            <p
              key={i}
              className="hidden lg:flex text-gray-700 text-[12px] lg:text-sm"
            >
              {p}
            </p>
          ))}
          ...
          <div className="flex items-center space-x-4 mt-4">
            <button
              onClick={prev}
              className="p-2 bg-white shadow hover:bg-gray-100 cursor-pointer"
            >
